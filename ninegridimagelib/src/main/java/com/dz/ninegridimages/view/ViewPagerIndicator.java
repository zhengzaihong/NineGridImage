...
public class ViewPagerIndicator extends android.view.View {
    ...
    private void setStyleable(Context context, AttributeSet attrs) {
        ...
        mIndicatorType = array.getInteger(R.styleable.ViewPagerIndicator_vpi_indicatorType, VpIndicatorType.CIRCLE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mDistanceType) {
            ...
            case DistanceType.BY_LAYOUT://布局等分
                if (mIndicatorType == VpIndicatorType.CIRCLE_LINE) {
        ...
        switch (mIndicatorType) {
            ...
            case VpIndicatorType.LINE://线
                ...
                break;
            case VpIndicatorType.CIRCLE_LINE://圆线
                ...
                break;
            case VpIndicatorType.BEZIER://贝塞尔
                for (int i = 0; i < mNum; i++) {//默认点 -(mNum - 1) * 0.5f * mDistance 第一个点
                    canvas.drawCircle(-(mNum - 1) * 0.5f * mDistance + i * mDistance, 0, mRadius, paintStroke);
                }
                //选中
                ...
                break;
            case VpIndicatorType.SPRING://贝塞尔 弹性
                for (int i = 0; i < mNum; i++) {//默认点 -(mNum - 1) * 0.5f * mDistance 第一个点
                    canvas.drawCircle(-(mNum - 1) * 0.5f * mDistance + i * mDistance, 0, mRadius, paintStroke);
                }
                drawSpringBezier(canvas);
                break;
            case VpIndicatorType.PROGRESS://进度条
                for (int i = 0; i < mNum; i++) {//默认点 -(mNum - 1) * 0.5f * mDistance 第一个点
                    canvas.drawCircle(-(mNum - 1) * 0.5f * mDistance + i * mDistance, 0, mRadius, paintStroke);
                }
                //选中
    ...
    public void move(float percent, int position, boolean isLeft) {
        ...
        switch (mIndicatorType) {
            ...
            case VpIndicatorType.BEZIER://贝塞尔
                ...
                break;
            case VpIndicatorType.SPRING://弹性
     ...
     * 线,圆,圆线,贝塞尔,弹性球,进度条
     */
    public interface VpIndicatorType {
        int LINE = 0;
        int CIRCLE = 1;
        int CIRCLE_LINE = 2;
        int BEZIER = 3;
     ...
     * @return
     */
    public ViewPagerIndicator setSelectColor(int color){
        this.mSelected_color = color;
        invalidate();
        return this;
    }
    /**
     ...
     * @return
     */
    public ViewPagerIndicator setDefaultColor(int color){
        this.mDefault_color = color;
        invalidate();
        return this;
    }
     ...
     * @return
     */
    public ViewPagerIndicator setRadiusSelected(float radiusSelected){
        this.mRadiusSelected = radiusSelected;
        invalidate();
        return this;
    }
     ...
     * @return
     */
    public ViewPagerIndicator setLength(int mLength){
     ...
     * @return
     */
    public ViewPagerIndicator setAnimation(boolean animation){
